...
public class TMallActivity extends BaseActivity implements ListView.OnPullRefreshListener, ListView.OnLoadMoreListener {
...
//        basePage.setId(1l);
        ...
        requestEntity = new RequestEntity.Builder(ApiPath.MALL_LIST.path()).addParams(pageReq).build();
        new HttpRequest<PageRsp<TMall, PageResult<TMall>>>(this, requestEntity) {
            @Override
            protected void onSuccess(PageRsp<TMall, PageResult<TMall>> result) {
    ...
    public void getTMall(Long mallId) {
        HashMap<String, Long> mallIdMap = new HashMap<>();
        mallIdMap.put("id", mallId);
        BaseReq<HashMap<String, Long>> baseReq = new BaseReq(mallIdMap);
        requestEntity = new RequestEntity.Builder(ApiPath.MALL_GET.path()).addParams(baseReq).build();
        new HttpRequest<BaseRsp<TMall>>(this, requestEntity, getToastDialog()) {
            @Override
            protected void onSuccess(BaseRsp<TMall> result) {
                super.onSuccess(result);
                Log.i("success\n" + JsonUtils.toJsonViewStr(result));
            }
