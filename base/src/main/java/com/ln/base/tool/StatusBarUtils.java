...
//                attributes.flags |= flagTranslucentStatus;
    ...
    public StatusBarUtils defaultDrawable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            setDrawable(mActivity.getResources().getDrawable(R.drawable.bg_empty, mActivity.getTheme()));
            //setDrawable(mActivity.getResources().getDrawable(R.drawable.bg_base_gradient_horizontal,mActivity.getTheme()));
        } else {
            setDrawable(mActivity.getResources().getDrawable(R.drawable.bg_empty));
            //setDrawable(mActivity.getResources().getDrawable(R.drawable.bg_base_gradient_horizontal,mActivity.getTheme()));
        }
        return this;
    }

    public StatusBarUtils defaultColor() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            setColor(mActivity.getResources().getColor(R.color.white, mActivity.getTheme()));
        } else {
            setColor(mActivity.getResources().getColor(R.color.white));
        }
        return this;
    }

    public StatusBarUtils actionBar() {
        setIsActionBar(true);
        clearActionBarShadow();
        return this;
    }

    public StatusBarUtils drawerLayout(@LayoutRes int contentResourceId) {
        setDrawerLayoutContentId(true, contentResourceId);
        return this;
    }
    ...
    public StatusBarUtils setColor(@ColorInt int color) {
        mColor = color;
        return this;
    }
    ...
    /**
     * 最外层布局是否为 DrawerLayout 的侧滑菜单
     *
     * @param drawerLayout      是否最外层布局为 DrawerLayout
     * @param contentResourceId 内容视图的 id
     * @return
     */
    public StatusBarUtils setDrawerLayoutContentId(boolean drawerLayout, @LayoutRes int contentResourceId) {
        mIsDrawerLayout = drawerLayout;
        mContentResourceIdInDrawer = contentResourceId;
        return this;
    }
     ...
     * @param activity
     */
    private void addStatusViewWithColor(Activity activity, @ColorInt int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isDrawerLayout()) {
                ...
                DrawerLayout drawer = (DrawerLayout) parentView;
                //内容视图
                View content = activity.findViewById(mContentResourceIdInDrawer);
    ...
    private void fullScreen(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                //导航栏颜色也可以正常设置
//                window.setNavigationBarColor(Color.TRANSPARENT);
                //Android 6.0以后，我们可以使用代码将状态栏字体的颜色设置为黑色
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
