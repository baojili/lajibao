...
public class SwitchButton extends CompoundButton {
    ...
    private float minChangeDistanceScale = 0.3f; // 有效距离比例，例如按钮宽度为100，比例为0.3，那么只有当滑动距离大于等于(100*0.3)才会切换状态，否则就回滚
    ...
    private void init(AttributeSet attrs) {
        ...
        frameDrawable = getResources().getDrawable(R.drawable.switch_frame);
        stateDrawable = getResources().getDrawable(R.drawable.switch_state);
        stateMaskDrawable = getResources().getDrawable(R.drawable.switch_mask);
        sliderDrawable = getResources().getDrawable(R.drawable.switch_slider);
        ...
        stateMaskDrawable.setBounds(0, 0, stateMaskDrawable.getIntrinsicWidth(), stateMaskDrawable.getIntrinsicHeight());
        sliderDrawable.setBounds(0, 0, sliderDrawable.getIntrinsicWidth(), sliderDrawable.getIntrinsicHeight());

        tempMinSlideX = (-1 * (stateDrawable.getIntrinsicWidth() - frameDrawable.getIntrinsicWidth())); // 初始化X轴最小值
...
//				int src = canvas.saveLayer(0, 0, getWidth(), getHeight(), paint, Canvas.MATRIX_SAVE_FLAG | Canvas.CLIP_SAVE_FLAG | Canvas.HAS_ALPHA_LAYER_SAVE_FLAG | Canvas.FULL_COLOR_LAYER_SAVE_FLAG | Canvas.CLIP_TO_LAYER_SAVE_FLAG);
    ...
    public int getCompoundPaddingRight() {
        return super.getCompoundPaddingRight() + (frameDrawable != null ? frameDrawable.getIntrinsicWidth() : 0);
    }
